    ...
    def __init__(self, config, log=None):
        ...
        if config.quantum:
            ...
            if config.qc_pretrained:
                ...
                self.gen_weights = torch.tensor(list(np.random.normal(0, math.sqrt(1/config.layer), config.layer*config.qubits*3+config.qubits*2)), requires_grad=True)
    ...
    def train_or_valid(self, epoch_i, train_val_test='val'):
        ...
        for a_step in range(the_step):
            ...
            if self.mode == 'train':
                ...
                R=[list(x.cpu()[i]) + list(a.cpu()[i].reshape(-1)) for i in range(self.batch_size)]
                F=[list(nodes_hard.cpu()[i]) + list(edges_hard.cpu()[i].reshape(-1))  for i in range(self.batch_size)]
